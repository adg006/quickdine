      ...
      <main className="grow max-w-7xl w-full mx-auto px-6 md:px-10 py-12">
        <div className="grow space-y-10">
          <div className="pb-4 border-b border-outline-variant/10">
            <h2 className="font-display text-2xl md:text-3xl font-semibold text-primary">Welcome back, {user.name.split(" ")[0]}</h2>
              ...
              {loadingBookings ? (
                ...
                <div className="space-y-4">
                  {upcomingBookings.map((b) => (
                    <div key={b._id} className="bg-white border border-outline-variant/20 rounded-md p-6 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                      <div className="flex gap-4">
                        ...
                        <div className="space-y-1">
                          <span className="text-[9px] font-medium text-secondary tracking-widest uppercase">{b.restaurant?.cuisine}</span>
                          <h4 className="font-display text-base font-medium text-primary">{b.restaurant?.name}</h4>
                      ...
                      <div className="flex gap-3 w-full md:w-auto justify-end">
                        <button onClick={() => handleCancelBooking(b._id)} className="px-5 py-2.5 text-[10px] font-medium tracking-widest uppercase text-error hover:bg-error-container/20 border border-outline-variant/40 rounded-sm cursor-pointer transition-colors">
                          Cancel
                        </button>
                      </div>
                    </div>
                            ...
                            {pastBookings.map((b) => (
                              <tr key={b._id} className="hover:bg-surface/50">
                                <td className="p-4 font-medium text-primary">
                                  <Link to={`/restaurant/${b.restaurant?.slug}`} className="hover:text-secondary">
                                    {b.restaurant?.name}
                                  </Link>
                                </td>
                                <td className="p-4">
                                ...
                                <td className="p-4">
                                  <span className={`inline-block py-0.5 px-2 text-[9px] font-medium tracking-wider uppercase rounded-sm ${b.status === "confirmed" ? "bg-secondary-container/30 text-on-secondary-container" : b.status === "completed" ? "bg-green-100 text-green-800" : "bg-error-container text-on-error-container"}`}>{b.status}</span>
                                </td>
                              </tr>
                            ))}
                          </tbody>
