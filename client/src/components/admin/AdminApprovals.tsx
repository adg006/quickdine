  ...
  return (
    <div className="space-y-8 text-left">
      <div className="space-y-4">
        <h3 className="font-display text-lg font-medium text-primary flex items-center gap-2">Pending Registration Requests ({pendingRestaurants.length})</h3>
        ...
        {pendingRestaurants.length === 0 ? (
          ...
          <div className="space-y-4">
            {pendingRestaurants.map((r) => (
              <div key={r._id} className="bg-white border border-outline-variant/20 rounded-md p-6 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                ...
                <div className="flex gap-2 shrink-0 w-full md:w-auto justify-end">
                  <button disabled={btnLoading === r._id} onClick={() => onApproveStatus(r._id, "approved")} className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-[9px] font-medium tracking-wider uppercase transition-colors rounded-sm cursor-pointer disabled:opacity-50">
                    Approve
                  </button>

                  <button disabled={btnLoading === r._id} onClick={() => onApproveStatus(r._id, "rejected")} className="px-4 py-2 bg-error hover:bg-error/85 text-white text-[9px] font-medium tracking-wider uppercase transition-colors rounded-sm cursor-pointer disabled:opacity-50">
                    Reject
                  </button>
                </div>
              </div>
                ...
                {otherRestaurants.map((r) => (
                  <tr key={r._id} className="hover:bg-surface/50">
                    ...
                    <td className="p-4 text-right space-x-3">
                      <span className={`inline-block py-0.5 px-2 text-[9px] font-medium tracking-wider uppercase rounded-sm ${r.status === "approved" ? "bg-green-100 text-green-800" : "bg-error-container text-on-error-container"}`}>{r.status}</span>

                      {r.status === "approved" ? (
                        <button onClick={() => onApproveStatus(r._id, "rejected")} className="text-error hover:underline text-[10px] uppercase font-medium cursor-pointer">
                          Suspend
                        </button>
                      ) : (
                        <button onClick={() => onApproveStatus(r._id, "approved")} className="text-green-600 hover:underline text-[10px] uppercase font-medium cursor-pointer">
                          Re-Approve
                        </button>
                      )}
                    </td>
