  ...
  return (
    <div className="group relative bg-white border border-outline-variant/10 card-hover-effect overflow-hidden rounded-md flex flex-col h-full">
      <Link to={`/restaurant/${restaurant.slug}`} className="relative h-60 overflow-hidden block">
        <img src={restaurant.image} alt={restaurant.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" loading="lazy" />
        <div className="absolute inset-0 bg-linear-to-t from-black/30 via-transparent to-transparent"></div>

        <div className="absolute bottom-4 left-4 flex flex-wrap gap-2">
          {restaurant.exclusive && <span className="text-[9px] font-medium tracking-widest text-white bg-secondary py-1 px-2.5 uppercase">EXCLUSIVE</span>}

          {restaurant.featured && <span className="text-[9px] font-medium tracking-widest text-on-primary bg-primary py-1 px-2.5 uppercase">RECOMMENDED</span>}
        </div>
      </Link>
      ...
      <div className="p-5 flex-1 flex flex-col justify-between">
        <div>
          ...
          <Link to={`/restaurant/${restaurant.slug}`} className="block mb-2">
            <h3 className="font-display text-lg font-semibold text-primary group-hover:text-secondary transition-colors line-clamp-1">{restaurant.name}</h3>
          </Link>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {restaurant.availableSlots
              ...
              .slice(0, 3)
              .map((slot) => (
                <button key={slot} onClick={(e) => handleSlotClick(e, slot)} className="text-[10px] font-medium border border-outline-variant/60 hover:border-primary px-3 py-1.5 transition-colors cursor-pointer text-black/55 hover:text-primary bg-surface">
                  {slot}
                </button>
              ))}

            <Link to={`/restaurant/${restaurant.slug}`} className="text-[10px] font-medium border border-outline-variant/20 px-3 py-1.5 transition-colors cursor-pointer text-secondary hover:bg-secondary hover:text-white">
              ALL SLOTS
            </Link>
          </div>
        </div>
