            ...
            {loadingSlots ? (
              ...
              (() => {
                ...
                return allSlots.filter((slotInfo: any) => {
                  ...
                  const [slotHour, slotMinute] = slotInfo.time
                    .split(":")
                    .map(Number);
                  const now = new Date();
                ...
                const isFull =
                  ...
                  slotInfo.availableSeats < Number(selectedGuests);
                return (
                  <button
                    key={slot}
                    type="button"
                    ...
                    className={`py-2 px-1 text-center text-[10px] font-medium tracking-wider uppercase border transition-all rounded-sm ${isSelected ? "bg-secondary border-secondary text-white shadow-sm cursor-pointer" : isFull ? "bg-black/5 border-outline-variant/10 text-black/25 cursor-not-allowed opacity-50" : "border-outline-variant/40 text-black/55 hover:border-primary hover:text-primary cursor-pointer"}`}
                  >
                    {slot}
                    {isFull && (
